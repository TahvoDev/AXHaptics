...
namespace VRCOSC.Modules.AXHaptics.Axis
{
    public class UdpSocket : IDisposable
    {
        ...
        protected Span<byte> DataIn =>
            _dataInLength > 0 ? _dataInBuffer.AsSpan(0, _dataInLength) : Array.Empty<byte>();
        protected Span<byte> MessageIn =>
            _messageInLength > 0 ? _messageInBuffer.AsSpan(0, _messageInLength) : Array.Empty<byte>();
        ...
        public virtual void Stop()
        {
            ...
            // Prevents block when waiting _rawReceiveTask and _messageReceiveTask
            _rawReceiveTask?.ContinueWith(task => {
                _messageReceiveTask?.ContinueWith(task => {
